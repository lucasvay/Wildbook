...
public class GridManager {
    ...
    private static ConcurrentHashMap<String, EncounterLite> matchGraph = new
        ConcurrentHashMap<String, EncounterLite>();
    ...
    private ArrayList<ScanWorkItemResult> done = new ArrayList<
        ScanWorkItemResult>();
     ...
     *  return false;
     * }*/
    public synchronized boolean isGridSpaceAvailable(HttpServletRequest request,
        boolean targeted) {
        String nodeID = request.getParameter("nodeIdentifier");
    ...
    private void updateGridStats(String context) {
        ...
        if ((lastGridStatsQuery == 1) || ((currenTime - lastGridStatsQuery) >
            gridStatsRefreshPeriod)) {
    ...
    public void removeCompletedWorkItemsForTask(String taskID) {
        ...
        try {
            for (int i = 0; i < done.size(); i++) {
                if ((done.get(i) != null) && (done.get(i).getUniqueNumberTask().
                        equals(taskID))) {
                    done.remove(i);
                    i--;
                    // iter--;
    ...
    public boolean doneContains(ScanWorkItemResult swir) {
        ...
        try {
            ...
            for (int i = 0; i < iter; i++) {
                if ((done.get(i) != null) && (done.get(i).
                        getUniqueNumberWorkItem().equals(swir.
                        getUniqueNumberWorkItem())))
                    hasit = true;
            }
        } catch (Exception e) {}
        return hasit;
    ...
    public int getNumWorkItemsIncompleteForTask(String taskID) {
        ...
        try {
            ...
            for (int i = 0; i < toDo.size(); i++) {
                if ((toDo.get(i) != null) && (toDo.get(i).getTaskIdentifier().
                        equals(taskID)))
                    num++;
            }
            ...
            for (int i = 0; i < iter; i++) {
                if ((underway.get(i) != null) && (underway.get(i).
                        getTaskIdentifier().equals(taskID)))
                    num++;
            }
        } catch (Exception e) { e.printStackTrace(); }
        return num;
    ...
    public static ConcurrentHashMap<String, EncounterLite> getMatchGraph() {
        return matchGraph;
    }
    ...
    public static synchronized int getNumRightPatterns() {
        return numRightPatterns;
    }
    public static synchronized int getNumLeftPatterns() {
        return numLeftPatterns;
    }


    private static ConcurrentHashMap<String, Long> speciesCountsMapLeft = new
        ConcurrentHashMap<String, Long>();
    private static ConcurrentHashMap<String, Long> speciesCountsMapRight = new
        ConcurrentHashMap<String, Long>();
        ...
        while (keys.hasMoreElements()) {
            ...
            if ((el.getSpots() != null) && (el.getSpots().size() > 0)) {
                ...
                if (speciesCountsMapLeft.containsKey(species))
                    speciesCountsMapLeft.put(species, Long.sum(
                        speciesCountsMapLeft.get(species), 1));
            ...
            if ((el.getRightSpots() != null) && (el.getRightSpots().size() > 0))
                ...
                if (speciesCountsMapRight.containsKey(species))
                    speciesCountsMapRight.put(species, Long.sum(
                        speciesCountsMapRight.get(species), 1));
    ...
    public void resetMatchGraphWithInitialCapacity(int initialCapacity) {
        matchGraph = null;
        matchGraph = new ConcurrentHashMap<String, EncounterLite>(
            initialCapacity);
    }
}
