...
public final class Geometry {
     ...
     */
    public static boolean isPointInsidePolygon(double[] x, double[] y, double
        ...
        for (int i = 0; i < nPoints; i++) {
            j++;
            if (j == nPoints) j = 0;
            if (y[i] < pointY && y[j] >= pointY || y[j] < pointY && y[i] >=
                pointY) {
                if (x[i] + (pointY - y[i]) / (y[j] - y[i]) * (x[j] - x[i]) <
                    pointX)
                    isInside = !isInside;
            }
        }
     ...
     * @return True if the given point is inside the polygon, false otherwise.
     */
    public static boolean isPointInsidePolygon(int[] x, int[] y, int pointX, int
        pointY) {
        boolean isInside = false;
        int nPoints = x.length;
        int j = 0;
        ...
        for (int i = 0; i < nPoints; i++) {
            ...
            if (y[i] < pointY && y[j] >= pointY || y[j] < pointY && y[i] >=
                pointY) {
                if (x[i] + (double)(pointY - y[i]) / (double)(y[j] - y[i]) * (x[
                    j] - x[i]) < pointX)
                    isInside = !isInside;
            }
        }
     ...
     */
    public static double[] computePointOnLine(double[] p0, double[] p1, double
        fractionFromP0) {
    ...
    public static double distance(int x0, int y0, int x1, int y1, int x, int y)
        ...
        double u = ((x - x0) * (x1 - x0) + (y - y0) * (y1 - y0)) / (length *
            length);
     ...
     *         >0 if points on same side.
     */
    private static int sameSide(int x0, int y0, int x1, int y1, int px0, int py0
        , int px1, int py1) {
        return sameSide((double)x0, (double)y0, (double)x1, (double)y1, (double)
                px0, (double)py0, (double)px1, (double)py1);
    }
    /**
     ...
     *         false otherwise.
     */
    public static boolean isLineIntersectingRectangle(int lx0, int ly0, int lx1,
        int ly1, int x0, int y0, int x1, int y1) {
     ...
     *         false otherwise.
     */
    public static boolean isPolylineIntersectingRectangle(int[] x, int[] y, int
        ...
        for (int i = 1; i < x.length; i++) {
            if (x[i - 1] != x[i] || y[i - 1] != y[i]) {
                if (Geometry.isLineIntersectingRectangle(x[i - 1], y[i - 1], x[i
                    ], y[i], x0, y0, x1, y1))
                    return true;
            }
        }
     ...
     */
    public static boolean isPolygonIntersectingRectangle(int[] x, int[] y, int
        x0, int y0, int x1, int y1) {
        int n = x.length;
        ...
        if (Geometry.isLineIntersectingRectangle(x[n - 2], y[n - 2], x[n - 1], y
            [n - 1], x0, y0, x1, y1))
            return true;
        //
        ...
        // or separate.
        //
        if (Geometry.isPointInsidePolygon(x, y, x0, y0) || Geometry.
     ...
     */
    public static int findLineSegmentIntersection(double x0, double y0, double
        ...
        double a0 = Geometry.equals(x0, x1, LIMIT) ? INFINITY : (y0 - y1) / (x0
            - x1);
        double a1 = Geometry.equals(x2, x3, LIMIT) ? INFINITY : (y2 - y3) / (x2
            - x3);
        double b0 = y0 - a0 * x0;
        double b1 = y2 - a1 * x2;
        ...
        if (Geometry.equals(a0, a1)) {
            if (!Geometry.equals(b0, b1)) {
                ...
                if (Geometry.equals(x0, x1)) {
                    if (Math.min(y0, y1) < Math.max(y2, y3) || Math.max(y0, y1)
                        > Math.min(y2, y3)) {
                        double twoMiddle = y0 + y1 + y2 + y3 - Geometry.min(y0,
                            y1, y2, y3) - Geometry.max(y0, y1, y2, y3);
                    ...
                    if (Math.min(x0, x1) < Math.max(x2, x3) || Math.max(x0, x1)
                        > Math.min(x2, x3)) {
                        double twoMiddle = x0 + x1 + x2 + x3 - Geometry.min(x0,
                            x1, x2, x3) - Geometry.max(x0, x1, x2, x3);
        ...
        if (Geometry.equals(x0, x1)) {
            if (y0 < y1)
                distanceFrom1 = y<y0 ? Geometry.length(x, y, x0, y0) : y> y1 ?
                    Geometry.length(x, y, x1, y1) : 0.0;
            else
                distanceFrom1 = y<y1 ? Geometry.length(x, y, x1, y1) : y> y0 ?
                    Geometry.length(x, y, x0, y0) : 0.0;
        } else {
            if (x0 < x1)
                distanceFrom1 = x<x0 ? Geometry.length(x, y, x0, y0) : x> x1 ?
                    Geometry.length(x, y, x1, y1) : 0.0;
            else
                distanceFrom1 = x<x1 ? Geometry.length(x, y, x1, y1) : x> x0 ?
                    Geometry.length(x, y, x0, y0) : 0.0;
        }
        ...
        if (Geometry.equals(x2, x3)) {
            if (y2 < y3)
                distanceFrom2 = y<y2 ? Geometry.length(x, y, x2, y2) : y> y3 ?
                    Geometry.length(x, y, x3, y3) : 0.0;
            else
                distanceFrom2 = y<y3 ? Geometry.length(x, y, x3, y3) : y> y2 ?
                    Geometry.length(x, y, x2, y2) : 0.0;
        } else {
            if (x2 < x3)
                distanceFrom2 = x<x2 ? Geometry.length(x, y, x2, y2) : x> x3 ?
                    Geometry.length(x, y, x3, y3) : 0.0;
            else
                distanceFrom2 = x<x3 ? Geometry.length(x, y, x3, y3) : x> x2 ?
                    Geometry.length(x, y, x2, y2) : 0.0;
        }
        return Geometry.equals(distanceFrom1, 0.0) && Geometry.equals(
                   distanceFrom2, 0.0) ? 1 : 0;
    }
    /**
    ...
    public static double[] findLinePolygonIntersections(double[] x, double[] y,
        ...
        for (int i = 0; i < nPoints; i++) {
            ...
            if (type == -2) {           // Overlapping
                ...
                int side = Geometry.sameSide(x0, y0, x1, y1, x[p1], y[p1], x[p2]
                    , y[p2]);
    ...
    public static int[] createEllipse(int[] x, int[] y) {
        ...
        // it to zero as well.
        if (!Geometry.equals(axisAngle, Math.PI / 2.0, 0.1) && !Geometry.equals(
            axisAngle, Math.PI, 0.1) && !Geometry.equals(axisAngle, 0.0, 0.1)) {
     ...
     * @return Ellipse geometry [x,y,x,y,...].
     */
    public static double[] createEllipse(double x0, double y0, double dx, double
        dy) {
        // Make sure deltas are positive
        dx = Math.abs(dx);
        dy = Math.abs(dy);
     ...
     */
    public static int[] createSector(int x0, int y0, int dx, int dy, double
        angle0, double angle1) {
     ...
     */
    public static int[] createArrow(int[] x, int[] y, double length, double
        angle, double inset) {
        ...
        v = dx > 0.0 && dy <= 0.0 ? Math.PI + v : dx > 0.0 && dy >= 0.0 ? Math.
                PI - v : dx <= 0.0 && dy<0.0 ? -v : dx <= 0.0 && dy> 0.0 ? +v :
            0.0;
     ...
     */
    public static int[] createArrow(int x0, int y0, int x1, int y1, double
        length, double angle, double inset) {
        int[] x = { x0, x1 };
        int[] y = { y0, y1 };
    ...
    public static int[] createRectangle(int x0, int y0, int width, int height) {
        return new int[] {
            x0, y0, x0 + (width - 1), y0, x0 + (width - 1), y0 + (height
                - 1), x0, y0 + (height - 1), x0, y0
        };
    }
    /**
     * Create geometry for a rectangle. Returns a closed polygon; first
    ...
    public static double[] createRectangle(double x0, double y0, double width,
        double height) {
        return new double[] {
            x0, y0, x0 + width, y0, x0 + width, y0 + height, x0, y0 +
                height, x0, y0
        };
    }
    /**
     ...
     */
    public static int[] createStar(int x0, int y0, int innerRadius, int
        outerRadius, int nArms) {
    ...
    public static boolean findPolygonPosition(int[] x, int[] y, double length,
        ...
        for (int i = 1; i < x.length; i++) {
            ...
            if (legLength + accumulatedLength >= length) {
                ...
                position[0] = (int)Math.round(x[i - 1] + fraction * (x[i] - x[i
                    - 1]));
                position[1] = (int)Math.round(y[i - 1] + fraction * (y[i] - y[i
                    - 1]));
                return true;
            }
