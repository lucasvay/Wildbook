...
public class ScanWorkItem implements java.io.Serializable {
    ...
    public ScanWorkItem(Encounter newEnc, Encounter existingEnc, String
        uniqueNum, String taskID, Properties props) {
        ...
        this.maxTriangleRotation = new Double(props.getProperty(
            "maxTriangleRotation"));
    ...
    public ScanWorkItem(EncounterLite newEnc, EncounterLite existingEnc, String
        uniqueNum, String taskID, Properties props) {
    ...
    public MatchObject execute() {
        ...
        if (!rightScan) {
            newspotsTemp = (SuperSpot[])newEncounter.getSpots().toArray(
                newspotsTemp);
            oldspotsTemp = (SuperSpot[])existingEncounter.getSpots().toArray(
                oldspotsTemp);
        } else {
            newspotsTemp = (SuperSpot[])newEncounter.getRightSpots().toArray(
                newspotsTemp);
            oldspotsTemp = (SuperSpot[])existingEncounter.getRightSpots().
                    toArray(oldspotsTemp);
        }
        ...
        for (int t = 0; t < spotLength; t++)
            newGrothSpots.add(new SuperSpot(new Spot(0, newspotsTemp[t].
                    getTheSpot().getCentroidX(), newspotsTemp[t].getTheSpot().
                    getCentroidY())));
        ...
        for (int t = 0; t < spotLength2; t++)
            existingGrothSpots.add(new SuperSpot(new Spot(0, oldspotsTemp[t].
                    getTheSpot().getCentroidX(), oldspotsTemp[t].getTheSpot().
                    getCentroidY())));
        MatchObject result = existingEncounter.getPointsForBestMatch(
            newspotsTemp, epsilon.doubleValue(), R.doubleValue(), Sizelim.
                doubleValue(), maxTriangleRotation.doubleValue(), C.doubleValue
            (), secondRun, rightScan);
        ...
        com.reijns.I3S.Point2D[] comapare2mePoints = new com.reijns.I3S.Point2D[
            0];
        com.reijns.I3S.Point2D[] lookForThisEncounterPoints = new com.reijns.I3S
                .Point2D[0];
        // if(rightScan){
