...
public class AppletHeartbeatThread implements Runnable, ISharkGridThread {
     ...
     */
    public AppletHeartbeatThread(String appletID, int numProcessors, String
        thisURLRoot, String version) {
    ...
    private void sendHeartbeat(String appletID) {
        ...
        try {
            u = new URL(rootURL + "/GridHeartbeatReceiver?nodeIdentifier=" +
                appletID + "&numProcessors=" + numProcessors + "&version=" +
                version);

            System.out.println("...sending heartbeat...thump...thump...to: " + u
                    .toString());
            ...
            System.out.println(
                "!!!!!I hit a MalformedURLException in the heartbeat thread!!!!!");
            mue.printStackTrace();
            // System.exit(0);
        } catch (IOException ioe) {
            System.out.println(
                "!!!!!I hit an IO exception in the heartbeat thread!!!!!");
            ioe.printStackTrace();
            // System.exit(0);
        } catch (Exception e) {
            System.out.println(
                "!!!!!I hit an Exception in the heartbeat thread!!!!!");
            e.printStackTrace();
            // System.exit(0);
        } finally {
            try {
