...
public class ScanWorkItemCreationThread implements Runnable, ISharkGridThread {
     ...
     */
    public ScanWorkItemCreationThread(String taskID, boolean rightSide, String
        encounterNum, boolean writeThis, String context, String jdoql) {
        this.taskID = taskID;
        this.writeThis = writeThis;
        this.rightSide = rightSide;
        this.encounterNumber = encounterNum;
        gm = GridManagerFactory.getGridManager();
        threadCreationObject = new Thread(this, ("scanWorkItemCreation_" +
            taskID));
    ...
    public void createThem() {
        ...
        EncounterLite baseEnc = gm.getMatchGraphEncounterLiteEntry(
            encounterNumber);
        ...
        try {
            ...
            while (keys.hasMoreElements()) {
                ...
                if (!kv.equals(encounterNumber) && baseEnc.doesSpeciesMatch(el))
                        {
                    String wiIdentifier = taskID + "_" + (new Integer(count)).
                            toString();


                    if (rightSide && (el.getRightSpots() != null) && (el.
                            getRightSpots().size() > 0)) {
                        // add the workItem
                        ScanWorkItem swi = new ScanWorkItem(baseEnc, el,
                            wiIdentifier, taskID, props2);
                        ...
                        count++;
                    } else if (!rightSide && (el.getSpots() != null) && (el.
                            getSpots().size() > 0)) {
                        // add the workItem
                        ScanWorkItem swi = new ScanWorkItem(baseEnc, el,
                            wiIdentifier, taskID, props2);
