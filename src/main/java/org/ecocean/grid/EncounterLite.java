...
public class EncounterLite implements java.io.Serializable {
    ...
    public EncounterLite(Encounter enc) {
        ...
        if (enc.getGenus() != null) {
            ...
            if (enc.getSpecificEpithet() != null)
                this.specificEpithet = enc.getSpecificEpithet();
        }
    }
    ...
    public SuperSpot[] getRightReferenceSpots() {
        if (rightReferenceSpotsX != null) {
            ...
            for (int q = 0; q < length; q++)
                rightArray[q] = new SuperSpot(rightReferenceSpotsX[q],
                    rightReferenceSpotsY[q]);
            return rightArray;
        } else {
            return null;
        }
    ...
    public MatchObject getPointsForBestMatch(SuperSpot[] newspotsTemp, double
        epsilon, double R, double Sizelim, double maxTriangleRotation, double C,
        boolean secondRun, boolean rightScan) {
        System.out.println("\nNow comparing against encounter " +
            encounterNumber + " of " + belongsToMarkedIndividual + "...");
        try {
            ...
            for (int iterj = 0; iterj < newspots.length; iterj++) {
                ...
                newspots[iterj] = new SuperSpot(new Spot(replaceMe.getArea(), (
                    replaceMe.getCentroidX() / normFactorNew), (replaceMe.
                        getCentroidY() / normFactorNew)));
            }
            ...
            for (int iterj2 = 0; iterj2 < spots.length; iterj2++) {
                ...
                spots[iterj2] = new SuperSpot(new Spot(replaceMe.getArea(), (
                    replaceMe.getCentroidX() / normFactorCatalog), (replaceMe.
                        getCentroidY() / normFactorCatalog)));
            }
            ...
            ArrayList newTriangles = new ArrayList(numSpots * (numSpots - 1) * (
                numSpots - 2) / 6);
            ...
            for (int i = 0; i < newSpotArrayL; i++) {
                for (int j = i + 1; j < (newspots.length - 1); j++) {
                    ...
                    for (int k = j + 1; k < newArrayL; k++) {
                        SpotTriangle tempTriangle = new SpotTriangle(newspots[i]
                                .getTheSpot(), newspots[j].getTheSpot(),
                            newspots[k].getTheSpot(), epsilon);
            ...
            ArrayList baseTriangles = new ArrayList(spotAL * (spotAL - 1) * (
                spotAL - 2) / 6);
            ...
            for (int i = 0; i < spotArrayL; i++) {
                for (int j = i + 1; j < (baseSpots.length - 1); j++) {
                    for (int k = j + 1; k < baseSpots.length; k++) {
                        SpotTriangle tempTriangle = new SpotTriangle(baseSpots[i
                            ].getTheSpot(), baseSpots[j].getTheSpot(), baseSpots
                            [k].getTheSpot(), epsilon);
            ...
            for (int iter1 = 0; iter1 < arrayL; iter1++) {
                ...
                for (int iter2 = 0; iter2 < baseArrayL; iter2++) {
                    ...
                    if ((RB > (RA - sqrttR2sum)) && (RB < (RA + sqrttR2sum))) {
                        ...
                        rotdiff = Math.abs(RotA - baseArray[iter2].
                                getMyVertexOneRotationInRadians()) /
                            allowedRotationDiff;
            ...
            for (int iter3 = 0; iter3 < VmatchesA.size(); iter3++) {
                logM.add(new Double((((SpotTriangle)VmatchesA.get(iter3)).
                        logPerimeter) - ((SpotTriangle)VmatchesB.get(iter3)).
                        logPerimeter));
            ...
            while (stillIterate && (numIterations < 20) && (VmatchesA.size() > 0
                ...
                for (int iter4 = 0; iter4 < logMSize; iter4++) {
                    boolean Aorientation = ((SpotTriangle)VmatchesA.get(iter4)).
                            clockwise;
                    boolean Borientation = ((SpotTriangle)VmatchesB.get(iter4)).
                            clockwise;
                ...
                for (int iter5 = 0; iter5 < logMSize; iter5++) {
                    boolean Aorientation = ((SpotTriangle)VmatchesA.get(iter5)).
                            clockwise;
                    boolean Borientation = ((SpotTriangle)VmatchesB.get(iter5)).
                            clockwise;


                    if (Aorientation == Borientation)
                        stdDeviationLogM += Math.pow((((Double)logM.get(iter5)).
                                doubleValue() - meanLogM), 2);
                }
                ...
                if (nPLUS > 1)
                    stdDeviationLogM = Math.pow((stdDeviationLogM / (nPLUS - 1))
                        , 0.5);
                else
                ...
                if (Math.pow((lessThanMeanLogM - greaterThanMeanLogM), 2) > (
                    lessThanMeanLogM + greaterThanMeanLogM)) {
                ...
                for (int iter6 = 0; iter6 < logM.size(); iter6++) {
                    if (Math.abs(((Double)logM.get(iter6)).doubleValue() -
                        meanLogM) > (multiple * stdDeviationLogM)) {
                        if (leftSideHeavy && (((Double)logM.get(iter6)).
                                doubleValue() < meanLogM))
                            leftsideRemovals++;
                        else if (rightSideHeavy && (((Double)logM.get(iter6)).
                                doubleValue() > meanLogM))
                            rightsideRemovals++;
                        else if (leftSideHeavy && (((Double)logM.get(iter6)).
                                doubleValue() > meanLogM))
                            rightsideRemovals++;
                        else if (rightSideHeavy && (((Double)logM.get(iter6)).
                                doubleValue() < meanLogM))
            ...
            if (VmatchesA.size() == 0)
                return new MatchObject(belongsToMarkedIndividual, 0, 0,
                        encounterNumber);
            ...
            for (int iter10 = 0; iter10 < vMatchL; iter10++) {
                for (int iter11 = 0; iter11 < 3; iter11++) {
                    Spot spotA = ((SpotTriangle)VmatchesA.get(iter10)).getVertex
                        (iter11 + 1);
                    Spot spotB = ((SpotTriangle)VmatchesB.get(iter10)).getVertex
                        (iter11 + 1);
            ...
            while (keepOnCounting && (iter20 < scoresSize)) {
                ...
                if ((scores[iter20].points > (scores[(iter20 - 1)].points / 2))
                    ...
                    secondRunSpots.add(scores[iter20]);
                    countedSpotsA.add(new Spot(0, scores[iter20].newX, scores[
                        iter20].newY));
                    countedSpotsB.add(new Spot(0, scores[iter20].oldX, scores[
                        iter20].oldY));
                } else {
            ...
            scoredSpots = (VertexPointMatch[])(secondRunSpots.toArray(
                scoredSpots));
            ...
            for (int iter25 = 0; iter25 < scoredSpots.length; iter25++) {
                ...
                for (int iter26 = 0; iter26 < secondRunSpotsA.size(); iter26++)
                        {
                    if ((scoredSpots[iter25].newX == (((SuperSpot)
                        secondRunSpotsA.get(iter26)).getTheSpot().getCentroidX()
                        )) && (scoredSpots[iter25].newY == (((SuperSpot)
                        secondRunSpotsA.get(iter26)).getTheSpot().getCentroidY()
                        )))
                        matchListA = true;
                }
                if (!matchListA)
                    secondRunSpotsA.add(new SuperSpot(new Spot(0, scoredSpots[
                        iter25].newX, scoredSpots[iter25].newY)));
                boolean matchListB = false;
                for (int iter26 = 0; iter26 < secondRunSpotsB.size(); iter26++)
                        {
                    if ((scoredSpots[iter25].oldX == (((SuperSpot)
                        secondRunSpotsB.get(iter26)).getTheSpot().getCentroidX()
                        )) && (scoredSpots[iter25].oldY == (((SuperSpot)
                        secondRunSpotsB.get(iter26)).getTheSpot().getCentroidY()
                        )))
                        matchListB = true;
                }
                if (!matchListB)
                    secondRunSpotsB.add(new SuperSpot(new Spot(0, scoredSpots[
                        iter25].oldX, scoredSpots[iter25].oldY)));
            }
            ...
            secondNewSpots = (SuperSpot[])(secondRunSpotsA.toArray(
                secondNewSpots));
            ...
            secondBaseSpots = (SuperSpot[])(secondRunSpotsB.toArray(
                secondBaseSpots));
            ...
            if ((secondNewSpots.length > 3) && (secondBaseSpots.length > 3)) {
                ...
                secondPassSpots = secondGrothPass(secondNewSpots,
                    secondBaseSpots, epsilon, R, Sizelim, maxTriangleRotation, C
                    );
            ...
            for (int logMIter = 0; logMIter < logMSize; logMIter++)
                logMbreakdown[logMIter] = ((Double)(logM.get(logMIter))).
                        doubleValue();
            ...
            if (!swappedSpots) {
                for (int diters = 0; diters < secondPassSpots.length; diters++)
                    ...
                    secondPassSpots[diters].newX = secondPassSpots[diters].newX
                        * normFactorNew;
                    // System.out.println("changed to: "+secondPassSpots[diters].newX);
                    secondPassSpots[diters].newY = secondPassSpots[diters].newY
                        * normFactorNew;
                    // System.out.println("changed to: "+secondPassSpots[diters].oldX);
                    secondPassSpots[diters].oldX = secondPassSpots[diters].oldX
                        * normFactorCatalog;
                    // System.out.println("changed to: "+secondPassSpots[diters].oldX);
                    secondPassSpots[diters].oldY = secondPassSpots[diters].oldY
                        * normFactorCatalog;
                }
            } else {
                for (int diters = 0; diters < secondPassSpots.length; diters++)
            ...
            if (swappedSpots) {
                VertexPointMatch[] fixedSpots = new VertexPointMatch[
                    secondPassSpots.length];


                for (int iter70 = 0; iter70 < secondPassSpots.length; iter70++)
                    fixedSpots[iter70] = new VertexPointMatch(secondPassSpots[
                        iter70].oldX, secondPassSpots[iter70].oldY,
                        secondPassSpots[iter70].newX, secondPassSpots[iter70].
                            newY, secondPassSpots[iter70].points);
            ...
            return new MatchObject(belongsToMarkedIndividual, bestScore,
                    adjustedScore, VmatchesA.size(), secondPassSpotsAL,
                    encounterNumber, pointBreakdown, logMbreakdown, sex, getDate
                    (), size);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private VertexPointMatch[] secondGrothPass(SuperSpot[] secondNewSpots,
        ...
        try {
            ...
            e.printStackTrace();
            System.out.println(
                "0 points awarded due to exception in secondGrothPass.");
            return scores;
        }
    }
    /**
    ...
    public I3SMatchObject i3sScan(EncounterLite newEnc, boolean scanRight) {
        ...
        if (scanRight) {
            newspotsTemp = (SuperSpot[])newEnc.getRightSpots().toArray(
                newspotsTemp);
        ...
        for (int i = 0; i < newSpotsLength; i++)
            newEncounterSpots[i] = new Point2D(newspotsTemp[i].getTheSpot().
                    getCentroidX(), newspotsTemp[i].getTheSpot().getCentroidY())
                ;
        ...
        for (int j = 0; j < thisSpotsLength; j++)
            thisEncounterSpots[j] = new Point2D(thisSpots[j].getTheSpot().
                    getCentroidX(), thisSpots[j].getTheSpot().getCentroidY());
        ...
        FingerPrint newPrint = new FingerPrint(newOrigEncounterSpots,
            newEncounterSpots, newEncControlSpots);
        ...
        FingerPrint thisPrint = new FingerPrint(origThisEncounterSpots,
            thisEncounterSpots, thisEncControlSpots);
        ...
        boolean successfulCompare = wsCompare.find(newPrint, fpBest, 1, true, hm
            ...
            return new I3SMatchObject(belongsToMarkedIndividual, fpBest[0].
                    getScore
                (), encounterNumber, sex, getDate(), size, hm, 0);
    }
    private void doAffine(FingerPrint fp) {
        double[] matrix = new double[6];

        Affine.calcAffine(fp.control[0].getX(), fp.control[0].getY(), fp.control
            [1].getX(), fp.control[1].getY(), fp.control[2].getX(), fp.control[2
            ].getY(), 100, 100, 900, 100, 500, 700, matrix);
        for (int i = 0; i < fp.orig.length; i++) {
            fp.fpp[i].x = matrix[0] * fp.orig[i].getX() + matrix[1] * fp.orig[i]
                    .getY() + matrix[2];
            fp.fpp[i].y = matrix[3] * fp.orig[i].getX() + matrix[4] * fp.orig[i]
                    .getY() + matrix[5];
        }
    ...
    public com.reijns.I3S.Point2D[] getThreeLeftFiducialPoints() {
        ...
        if ((getLeftReferenceSpots() != null) && (getLeftReferenceSpots().length
            == 3)) {
            SuperSpot[] refsLeft = getLeftReferenceSpots();


            Rray[0] = new com.reijns.I3S.Point2D(refsLeft[0].getTheSpot().
                    getCentroidX(), refsLeft[0].getTheSpot().getCentroidY());
            Rray[1] = new com.reijns.I3S.Point2D(refsLeft[1].getTheSpot().
                    getCentroidX(), refsLeft[1].getTheSpot().getCentroidY());
            Rray[2] = new com.reijns.I3S.Point2D(refsLeft[2].getTheSpot().
                    getCentroidX(), refsLeft[2].getTheSpot().getCentroidY());
            ...
            com.reijns.I3S.Point2D topLeft = new com.reijns.I3S.Point2D(
                getLeftmostSpot(), getHighestSpot());
            com.reijns.I3S.Point2D bottomLeft = new com.reijns.I3S.Point2D(
                getLeftmostSpot(), getLowestSpot());
            com.reijns.I3S.Point2D bottomRight = new com.reijns.I3S.Point2D(
                getRightmostSpot(), getLowestSpot());
            Rray[0] = topLeft;
            Rray[1] = bottomLeft;
            Rray[2] = bottomRight;
    ...
    public com.reijns.I3S.Point2D[] getThreeRightFiducialPoints() {
        com.reijns.I3S.Point2D[] Rray = new com.reijns.I3S.Point2D[3];
        if ((getRightReferenceSpots() != null) && (getRightReferenceSpots().
                length == 3)) {
            SuperSpot[] refsRight = getRightReferenceSpots();


            Rray[0] = new com.reijns.I3S.Point2D(refsRight[0].getTheSpot().
                    getCentroidX(), refsRight[0].getTheSpot().getCentroidY());
            Rray[1] = new com.reijns.I3S.Point2D(refsRight[1].getTheSpot().
                    getCentroidX(), refsRight[1].getTheSpot().getCentroidY());
            Rray[2] = new com.reijns.I3S.Point2D(refsRight[2].getTheSpot().
                    getCentroidX(), refsRight[2].getTheSpot().getCentroidY());
        } else {
            com.reijns.I3S.Point2D topRight = new com.reijns.I3S.Point2D(
                getRightmostRightSpot(), getHighestRightSpot());
            com.reijns.I3S.Point2D bottomRight = new com.reijns.I3S.Point2D(
                getRightmostRightSpot(), getLowestRightSpot());
            com.reijns.I3S.Point2D bottomLeft = new com.reijns.I3S.Point2D(
                getLeftmostRightSpot(), getLowestRightSpot());
    ...
    public void processLeftReferenceSpots(ArrayList<org.ecocean.SuperSpot>
        initSpots) {
        int length = initSpots.size();
    ...
    public void processRightReferenceSpots(ArrayList<org.ecocean.SuperSpot>
        initSpots) {
        int length = initSpots.size();
    ...
    public String getBelongsToMarkedIndividual() {
        return belongsToMarkedIndividual;
    }
    ...
    public boolean doesSpeciesMatch(EncounterLite el) {
        if (this.getGenus() == null || this.getSpecificEpithet() == null)
            return false;
        if (el.getGenus() == null || el.getSpecificEpithet() == null)
            return false;
        if (this.getGenus().equals(el.getGenus()) && this.getSpecificEpithet().
                equals(el.getSpecificEpithet())) return true;
        return false;
    }
}
