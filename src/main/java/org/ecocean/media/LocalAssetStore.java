...
public class LocalAssetStore extends AssetStore {
     ...
     * root.
     */
    public LocalAssetStore(final String name, final Path root, final String
        webRoot, final boolean writable) {
     ...
     * appended.  If null, this store offers no web access to assets.
     */
    LocalAssetStore(final Integer id, final String name, final AssetStoreConfig
     ...
     */
    private static AssetStoreConfig makeConfig(final Path root, final String
        webRoot) {
 ...
 */
    ...
    public Path localPath(MediaAsset ma) {
        if (ma == null)
            throw new IllegalArgumentException("MediaAsset is null");
 ...
 */
    ...
    throws IOException {
        ...
        if (subpath == null)
            throw new IOException("no path passed in parameters");
    ...
    throws IOException {
        ...
        if (!(fromMA.getStore() instanceof LocalAssetStore) || !(toMA.getStore()
            instanceof LocalAssetStore))
            throw new IOException("invalid AssetStore type(s)");
        if (!this.writable)
            throw new IOException(this.name + " is a read-only AssetStore");
        throw new IOException(
                  "oops, LocalAssetStore.copyAsset() still not implemented. :/")
            ;                                   // TODO
    }
 ...
 */
    ...
    public static Path checkPath(final Path root, final Path path) {
        ...
        if (result.startsWith(".."))
            throw new IllegalArgumentException(
                      "Path not under given root (root=" + root.toString() +
                      "; path=" + path.toString() + ")");
        return result;
    }
    ...
    @Override public URL webURL(final MediaAsset ma) {
        if (ma == null)
            System.out.println("MediaAsset is null in LocalAssetStore.webURL");
 ...
 */
    public String getFilename(MediaAsset ma) {
        if ((ma.getParameters() != null) && (ma.getParameters().optString(
            "iaSourcePath", null) != null)) {
    ...
    @Override public JSONObject createParameters(File file, String grouping) {
        ...
        if (underRoot(file.toPath())) { // note: we ignore grouping here!
            ...
            if (grouping == null)
                grouping = Util.hashDirectories(Util.generateUUID(), File.
                        separator);
            p.put("path", root().toString() + File.separator + grouping + File.
