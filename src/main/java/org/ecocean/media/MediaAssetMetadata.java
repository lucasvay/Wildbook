...
public class MediaAssetMetadata implements java.io.Serializable {
    ...
    public JSONObject getAttributes() {
        if ((getData() == null) || (getData().optJSONObject("attributes") ==
            null)) return new JSONObject();
    ...
    public HashMap<String, String> findRecurse(String regex) {
        if ((getData() == null) || (getData().optJSONObject("exif") == null))
            return null;
        return _find(getData().getJSONObject("exif"), null, regex.toLowerCase())
            ;
    }
    private static HashMap<String, String> _find(JSONObject jobj, String
        keyPrefix, String regex) {
 ...
 */
    public DateTime getDateTime() {
        ...
        HashMap<String, String> matches = _find(getData().getJSONObject("exif"),
            null, ".*date.*");
// System.out.println("MediaAssetMetadata.getDateTime() ----> " + matches);
        ...
        SimpleDateFormat dateParser = new SimpleDateFormat(
            "yyyy:MM:dd HH:mm:ss");     // note: exif doesnt carry tz :(
...
// System.out.println("* MediaAssetMetadata.getDateTime(): " + key + "[" + val + "] -> " + dt);
            ...
            if (key.toLowerCase().indexOf("original") > -1)
                count.put(dt, count.get(dt) + 2);
            if (key.toLowerCase().indexOf("digitized") > -1)
                count.put(dt, count.get(dt) + 1);
        }
    ...
    private Double[] getGPSArray() {
        ...
        for (int i = 0; i < keys.length; i++) {
            ...
            if (exif.getJSONObject("GPS").optString(keys[i], null) != null) {
                if (i < 2)                                      // digital degrees
                    gps[i] = Util.latlonDMStoDD(exif.getJSONObject("GPS").
                            getString(keys[i]));
                else
                    gps[i] = _parseAltitude(exif.getJSONObject("GPS").getString(
                        keys[i]));
            }
        }
...
// System.out.println("alt -> " + alt);
        ...
        if ((parts.length == 1) || "".equals(parts[1]) || "m".equals(parts[1].
                toLowerCase().substring(0, 1))) return dbl;     // already in metres (we are hoping, not miles?)
        if ("f".equals(parts[1].toLowerCase().substring(0, 1)))
            return dbl / 0.3048;                                // goodbye, feet
        System.out.println(
