...
public class YouTubeAssetStore extends AssetStore {
    ...
    YouTubeAssetStore(final Integer id, final String name, final
        AssetStoreConfig config, final boolean writable) {
    ...
    throws IllegalArgumentException {
        if (idFromParameters(params) == null)
    ...
    @Override public void copyAsset(final MediaAsset fromMA, final MediaAsset
        toMA)
    throws IOException {
    ...
    @Override public DateTime getDateTime(MediaAsset ma) {
        if ((ma == null) || (ma.getMetadata() == null) || (ma.getMetadata().
                getData().optJSONObject("detailed") == null)) return null;


        String upd = ma.getMetadata().getData().getJSONObject("detailed").
                optString("upload_date", null);


        if ((upd == null) || (upd.length() != 8)) return null;  // is like YYYYMMDD
        try {
            return new DateTime(Integer.parseInt(upd.substring(0, 4)), Integer.
                        parseInt(upd.substring(4, 6)), Integer.parseInt(upd.
                        substring(6)), 0, 1);
        } catch (Exception ex) {}
        return null;
    }
    ...
    throws IOException {
        ...
        File dir = Files.createTempDirectory("youtube_get_" + id + "_" + ma.
                getId() + "_").toFile();
    ...
    public static boolean grabAndParse(Shepherd myShepherd, MediaAsset ma,
        boolean wait)
    throws IOException {
        boolean processing = ((ma.getDerivationMethod() != null) && ma.
                getDerivationMethod().optBoolean("_processing", false));
        int count = 0;
        int max = 100;
        ...
        while (processing && (count < max)) {
            ...
            processing = ((ma.getDerivationMethod() != null) && ma.
                    getDerivationMethod().optBoolean("_processing", false));
        }
            ...
            if (f.getName().matches(".+.info.json$")) {
                ...
                try {
                    ...
                    System.out.println("ERROR: " + ma +
                        " grabAndParse() failed to parse YouTube .info.json - "
                        + jex.toString());
                }
// System.out.println("detailed -> " + detailed);
    ...
    throws IOException {
        ...
        if (locP == null)
            throw new IOException(ma +
                    " failed to get .localPath(); extractFrames() aborted");
        ...
        File fakeFile = new File(Util.hashDirectories(Util.generateUUID(), File.
                separator));
        ...
        if (dirP == null)
            dirP = Files.createTempDirectory("youtube_extract_" + ma.getId());  // fallback to tmp
    ...
    public static boolean extractFramesAndParse(Shepherd myShepherd, MediaAsset
        ma, boolean wait)
    throws IOException {
        if (ma == null)
        ...
        ArrayList<MediaAsset> vidKids = ma.findChildrenByLabel(myShepherd,
            "_video");


        if ((vidKids == null) || (vidKids.size() < 1))
            throw new IOException("YouTubeAssetStore.extractFramesAndParse(" +
                    ma + ") has no _video child");
        ...
        for (File f : extracted) {
            ...
            System.out.println("- [" + f.getName() +
                "] extractFramesAndParse: " + f);
            _createFrameChild(myShepherd, vidKids.get(0), f);
        }
        ma.addDerivationMethod("_processing", false);
        return true;
    }
    private static MediaAsset _createThumbChild(Shepherd myShepherd, MediaAsset
        parent, File f)
    throws java.io.IOException {
        if ((f == null) || !f.exists()) {
            System.out.println("WARNING: could not create thumb child for " +
                parent + ", file failure on " + f + "; skipping");
            return null;
        }
...
// System.out.println("thumb child created: " + kid);
    ...
    private static MediaAsset _createVideoChild(Shepherd myShepherd, MediaAsset
        parent, File f)
    throws java.io.IOException {
        if ((f == null) || !f.exists()) {
            System.out.println("WARNING: could not create video child for " +
                parent + ", file failure on " + f + "; skipping");
            return null;
        }
...
// System.out.println("video child created: " + kid);
    ...
    private static MediaAsset _createFrameChild(Shepherd myShepherd, MediaAsset
        parent, File f)
    throws java.io.IOException {
        if ((f == null) || !f.exists()) {
            System.out.println("WARNING: could not create frame child for " +
                parent + ", file failure on " + f + "; skipping");
            return null;
        }
        ...
        try {
            ...
            System.out.println("WARNING: YouTubeAssetStore._createFrameChild(" +
                parent + ") could not parse offset int value from " + f +
                " filename; using -1: " + ex.toString());
        }
...
// System.out.println("frame child created: " + kid);
    ...
    throws IOException {
        ...
        try {
            data.put("basic", YouTube.simpleInfo(idFromParameters(ma.
                    getParameters())));
        } catch (Exception ex) {
            System.out.println(ma + " failed simpleInfo(): " + ex.toString());
        }
        if (!minimal) {
            data.put("detailed", new JSONObject(
                "{\"_processing\": true, \"timestamp\": " + System.
                    currentTimeMillis() + "}"));        // assume it will be.... soon?
    ...
    public static YouTubeAssetStore find(Shepherd myShepherd) {
        ...
        if ((AssetStore.getStores() == null) || (AssetStore.getStores().size() <
            1)) return null;
    ...
    public static void backgroundGrabAndParse(final MediaAsset otherMa,
        ...
        Runnable rn = new Runnable() {
            public void run() {
                ...
                MediaAsset ma = MediaAssetFactory.load(otherMa.getId(),
                    myShepherd);
                ...
                try {
                    ...
                    System.out.println("ERROR: IOException grabbing " + ma +
                        ": " + ex.toString());
                }
                System.out.println("grabAndParse() -> " + ok + "; " + ((ma.
                        getMetadata() == null) ? "(null metadata)" : ma.
                        getMetadata().getDataAsString()));
    ...
    public static ArrayList<MediaAsset> findFrames(final MediaAsset ma, Shepherd
        myShepherd) {
        if (ma == null) return null;
