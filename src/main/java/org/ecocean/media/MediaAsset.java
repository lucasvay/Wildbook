 ...
 */
    ...
    public MediaAsset(final int id, final AssetStore store, final JSONObject
        params) {
        this.id = id;
        this.setUUID();
        this.store = store;
    ...
    public void setParameters(JSONObject p) {
        if (p == null) {
            System.out.println("WARNING: attempted to set null parameters on " +
                this + "; ignoring");
            return;
        }
        parameters = p;
    ...
    public void setParametersAsString(String p) {
        if (p == null) {
            System.out.println(
                "WARNING: attempted to set null parametersAsString on " + this +
                "; ignoring");
            return;
        }
    ...
    public void setDerivationMethodAsString(String d) {
        ...
        try {
            ...
            System.out.println(this +
                " -- error parsing derivation json string (" + d + "): " + je.
                    toString());
            derivationMethod = null;
        }
    }
...
// System.out.println("hashCode on " + this + " = " + this.hashCode);
    ...
    public void removeFeature(Feature f) {
        ...
        System.out.println("INFO: removeFeature() killing off " + f +
            " from asset id=" + this.id);
        features.remove(f);
    }
    ...
    public String getRotationInfo() {
        ...
        HashMap<String, String> orient = this.getMetadata().findRecurse(
            ".*orient.*");
    ...
    public Feature generateFeatureFromBbox(double w, double h, double x, double
        y, JSONObject params) {
 ...
 */
    ...
    public URL webURL() {
        if (store == null) {
            System.out.println("MediaAsset " + this.getUUID() +
                " has no store!");
            return null;
        }
        try {
            int i = ((store.getUsage() == null) ? -1 : store.getUsage().indexOf(
                "PLACEHOLDERHACK:"));
 ...
 */
    ...
    public URL safeURL(Shepherd myShepherd, HttpServletRequest request, String
        bestType) {
    ...
    public URL containerURLIfPresent() {
        String containerName = CommonConfiguration.getProperty("containerName",
            "context0");
        ...
        if (containerName != null && containerName != "") {
            try {
                System.out.println(
                    "Using containerName for MediaAsset URL domain..");
                return new URL(store.webURL(this).getProtocol(), containerName,
                        80, store.webURL(this).getFile());
            } catch (java.net.MalformedURLException ex) {}
        }
        try {
    ...
    public MediaAsset bestSafeAsset(Shepherd myShepherd, HttpServletRequest
        request, String bestType) {
        if (store == null) return null;
        ...
        if (parentId != null) {
            ...
            if (top == null)
                throw new RuntimeException(
                          "bestSafeAsset() failed to find parent on " + this);
...
// System.out.println("   ....  ??? do we have a " + t);
            ...
            ArrayList<MediaAsset> kids = top.findChildrenByLabel(myShepherd,
                "_" + t);
 ...
 */
    ...
    public org.datanucleus.api.rest.orgjson.JSONObject sanitizeJson(
        HttpServletRequest request, org.datanucleus.api.rest.orgjson.JSONObject
        jobj)
    ...
    public org.datanucleus.api.rest.orgjson.JSONObject sanitizeJson(
        HttpServletRequest request, org.datanucleus.api.rest.orgjson.JSONObject
        jobj, boolean fullAccess, Shepherd myShepherd)
    throws org.datanucleus.api.rest.orgjson.JSONException {
        ...
        if ((fts != null) && (fts.size() > 0)) {
            org.datanucleus.api.rest.orgjson.JSONArray jarr = new org.
                    datanucleus.api.rest.orgjson.JSONArray();
            for (int i = 0; i < fts.size(); i++) {
                org.datanucleus.api.rest.orgjson.JSONObject jf = new org.
                        datanucleus.api.rest.orgjson.JSONObject();
                ...
                try {                   // for some reason(?) this will get a jdo error for "row not found".  why???  anyhow, we catch it
                    ...
                    jf.put("type", "unknown");
                    System.out.println("ERROR: MediaAsset.sanitizeJson() on " +
                        this.toString() + " threw " + ex.toString());
                }
                ...
                if (ann != null) {
                    ...
                    if (enc != null) {
                        ...
                        if (enc.hasMarkedIndividual()) {
                            ...
                            String displayName = enc.getIndividual().
                                    getDisplayName(request, myShepherd);
                        ...
                        if (enc.getGenus() != null && enc.getSpecificEpithet()
                            != null) {
        ...
        if ((getMetadata() != null) && (getMetadata().getData() != null) && (
            ...
            jobj.put("metadata", Util.toggleJSONObject(getMetadata().getData().
        ...
        if (!jobj.optBoolean("_skipChildren", false))
            kids = this.findChildren(myShepherd);
        // myShepherd.rollbackDBTransaction();
        if ((kids != null) && (kids.size() > 0)) {
            org.datanucleus.api.rest.orgjson.JSONArray k = new org.datanucleus.
                    api.rest.orgjson.JSONArray();
            for (MediaAsset kid : kids)
                k.put(kid.sanitizeJson(request, new org.datanucleus.api.rest.
                        orgjson.JSONObject(), fullAccess, myShepherd));
            jobj.put("children", k);
        }
        if (fullAccess) {
        ...
        if ((this.getKeywords() != null) && (this.getKeywords().size() > 0)) {
            ...
            jobj.put("keywords", new org.datanucleus.api.rest.orgjson.JSONArray(
                ka.toString()));
        }
        // myShepherd.rollbackDBTransaction();
        // myShepherd.closeDBTransaction();
    ...
    throws IOException {
        ...
        if (type == null)
            throw new IOException("child does not have valid type");


        File sourceFile = (this.localPath() == null) ? null : this.localPath().
                toFile();
        File targetFile = (child.localPath() == null) ? null : child.localPath()
                .toFile();
        ...
        boolean ok = store._updateChildLocalWork(this, type, null, sourceFile,
            targetFile, false);


        System.out.println("INFO: redoChild() on parent=" + this + ", child=" +
            child + " => " + ok);
        return ok;
    }
    ...
    public ArrayList<MediaAsset> detachChildren(Shepherd myShepherd, String type
        ...
        ArrayList<MediaAsset> disposable = this.findChildrenByLabel(myShepherd,
            "_" + type);
        ...
        if ((disposable != null) && (disposable.size() > 0)) {
            for (MediaAsset ma : disposable) {
                ...
                System.out.println("INFO: detached child " + ma + " from " +
                this);
            }
        }
        return disposable;
    ...
    public ArrayList<MediaAsset> findChildrenByLabel(Shepherd myShepherd, String
        label) {
    ...
    public static MediaAsset findOneByLabel(ArrayList<MediaAsset> mas, Shepherd
    ...
    private static ArrayList<MediaAsset> findAllByLabel(ArrayList<MediaAsset>
        ...
        for (MediaAsset ma : mas) {
            ...
            ArrayList<MediaAsset> kids = ma.findChildrenByLabel(myShepherd,
                label);
    ...
    public ArrayList<MediaAsset> updateStandardChildren() {
        ...
        for (String type : types) {
            ...
            try {
                ...
                System.out.println("updateStandardChildren() failed on type=" +
                    type + ", ma=" + this + " with " + ex.toString());
            }
            if (c != null) mas.add(c);
        }
        return mas;
 ...
 */
    public static void updateStandardChildrenBackground(final String context,
        ...
        System.out.println("updateStandardChildrenBackground() [" + tid +
            "] forking for " + ids.size() + " MediaAsset ids >>>>");
        ...
        Runnable rn = new Runnable() {
            public void run() {
                ...
                for (Integer id : ids) {
                    ...
                    if (ma == null) continue;
                    ArrayList<MediaAsset> kids = ma.updateStandardChildren(
                        myShepherd);
                    System.out.println("+ [" + ct +
                        "] updateStandardChildrenBackground() [" + tid +
    ...
    public boolean hasKeyword(String keywordName) {
        if (keywords != null) {
            ...
            for (int i = 0; i < numKeywords; i++) {
                ...
                if ((keywordName.equals(kw.getIndexname()) || keywordName.equals
                    (kw.getDisplayName()))) return true;
            }
        }
        return false;
    }
    ...
    public String toHtmlElement(HttpServletRequest request, Shepherd myShepherd,
        ...
        if (store == null)
            return
                    "<!-- ERROR: MediaAsset.toHtmlElement() has no .store value for "
                    + this.toString() + " -->";
    ...
    public String[] getMimeType() {
        ...
        String mt = this.metadata.getAttributes().optString("contentType", null)
            ;                           // note: getAttributes always  returns a JSONObject (even if empty)
    ...
    public boolean hasMetadata() {
        ...
        return (data != null) && (data.getData() != null) && (data.getData().opt
                   ("attributes") != null);
    }
    ...
    public MediaAssetMetadata updateMinimalMetadata() {
        ...
        try {
            ...
            System.out.println("WARNING: updateMinimalMetadata() on " + this +
                " got " + ioe.toString() + "; failed to set");
            return null;
        }
        return metadata;
    }
    // handy cuz we dont need the actual file (if we have these values from elsewhere) and usually the only stuff we "need"
    public MediaAssetMetadata setMinimalMetadata(int width, int height, String
        contentType) {
    ...
    public void refreshIAStatus(Shepherd myShepherd) {
        String s = IBEISIA.parseDetectionStatus(Integer.toString(this.getId()),
            myShepherd);
    ...
    throws IOException {
        ...
        try {
            ...
            throw new IOException("copyInBase64() could not parse: " + ex.
                        toString());
        }
        ...
        try {
            ...
            System.out.println(
                "Exception from Writing FileOutputStream with imgBytes");
            e.printStackTrace();
        } finally {
            stream.close();
        }
 ...
 */
    public List<Encounter> assignEncounters(Shepherd myShepherd) {
        ...
        if (Util.collectionIsEmptyOrNull(annots)) {
            System.out.println("INFO: assignEncounters() finds no annots on " +
                this);
            return newEncs;
        }
        ...
        for (Annotation ann : annots) {
            ...
            if (ann.isTrivial() && (enc != null)) {     // i guess it would be weird to have trivial with no enc, but we dont want it!
                ...
                // what do we do here?
                System.out.println(
                    "INFO: assignEncounters() has no iaClass for " + ann);
            } else if (ann.getIAClass().indexOf("+") > -1) {    // we are a part, i guess?
                partCt.put(ann.getIAClass(), partCt.getOrDefault(ann.getIAClass
                    (), 0) + 1);
        ...
        System.out.println("INFO: assignEncounters() needsEncounter ==> " +
            needsEncounter);
        ...
        if (myEncs.isEmpty()) {
            ...
            if (needsEncounter.get(0).getIAClass() != null) {
                try {
                    ...
                    whichever.setTaxonomy(iaJson.taxonomyFromIAClass(
                        needsEncounter.get(0).getIAClass(), myShepherd));       // might work!
                } catch (Exception ex) {
                    System.out.println(
            ...
            System.out.println(
                "INFO: assignEncounters() has empty myEncs for " + this.toString
        ...
        System.out.println("INFO: assignEncounters() contiguousAnnots = " +
            contiguousAnnots);
        if ((nonPartCt > 1) || hasDuplicateParts || !contiguousAnnots || (myEncs
                .size() > 1)) {
            System.out.println(">>>>> assignEncounters() MANY ENCS ; myEncs=" +
                myEncs);
            /*
            ...
            for (Annotation ann : needsEncounter) {
                if (trivialAnnots.size() > 0) {
                    ...
                    System.out.println(
                        ">>>>> ******** [1] assignEncounters() trivial " + tann
                        + " replaced by " + ann + " on " + tenc);
                    if (!newEncs.contains(tenc)) newEncs.add(tenc);
                } else {                        // new enc based on whichever
                    try {
                        Encounter newEnc = whichever.cloneWithoutAnnotations(
                            myShepherd);        // ok, not really random
            ...
            System.out.println(">>>>> assignEncounters() ONE ENC ; myEncs=" +
                myEncs);
            for (Annotation ann : needsEncounter) {
                if (trivialAnnots.size() > 0) { // we have a trivial, lets replace it
                    ...
                    if (tenc != null) {
                        ...
                        tenc.resetDateInMilliseconds();
                        System.out.println(
                            ">>>>> ******** [2] assignEncounters() trivial " +
                            tann + " replaced by " + ann + " on " + tenc);
                        if (!newEncs.contains(tenc)) newEncs.add(tenc);
                    }
        ...
        System.out.println(">>>>> assignEncounters() EXIT ; size=" + newEncs.
                size() + " newEncs=" + newEncs);
        return newEncs;
    }
    ...
    public boolean equals(final Object o2) {
        ...
        if ((this.uuid == null) || (two == null) || (two.getUUID() == null))
            return false;
