...
public class TwitterAssetStore extends AssetStore {
    ...
    TwitterAssetStore(final Integer id, final String name, final
    ...
    @Override public URL webURL(final MediaAsset ma) {
        ...
        if (ma.hasLabel("_original")) {
            ...
            if (ma.getParameters() != null)
                u = ma.getParameters().optString("media_url", null);
        }
        if (u == null) return null;                     // dunno/how!
        try {
            return new URL(u);
        } catch (java.net.MalformedURLException ex) {   // "should never happen"
            System.out.println("TwitterAssetStore.webURL() on " + ma + ": " + ex
                    .toString());
            return null;
        }
    }
    ...
    private static Long longIdFromParameters(JSONObject params) {
        ...
        try {
            ...
            System.out.println(
                "TwitterStore.longIdFromParameters() failed to parse '" + id +
                "': " + ex.toString());
            return null;
        }
    }
...
// see:  http://twitter4j.org/javadoc/twitter4j/EntitySupport.html#getMediaEntities--
    public static List<MediaAsset> entitiesAsMediaAssets(MediaAsset ma) {
        ...
        if ((raw.optJSONObject("extended_entities") == null) || (raw.
                getJSONObject("extended_entities").optJSONArray("media") == null
            )) return null;


        List<MediaAsset> mas = new ArrayList<MediaAsset>();
        JSONArray jarr = raw.getJSONObject("extended_entities").getJSONArray(
            "media");
 ...
 */
    ...
    public static JSONObject getRawJSONObject(MediaAsset ma) {
        ...
        if ((md == null) || (md.getData() == null) || (md.getDataAsString() ==
            null)) return null;
 ...
 */
    private static void setEntityMetadata(MediaAsset ma) {
        ...
        if ((ma.getParameters().optJSONObject("sizes") != null) && (ma.
                getParameters().getJSONObject("sizes").optJSONObject("medium")
            != null)) {
            d.getJSONObject("attributes").put("width", ma.getParameters().
                    getJSONObject("sizes").getJSONObject("medium").optDouble(
                "w", 0));
            d.getJSONObject("attributes").put("height", ma.getParameters().
                    getJSONObject("sizes").getJSONObject("medium").optDouble(
                "h", 0));
        } else if ((ma.getParameters().optJSONObject("sizes") != null) && (ma.
                getParameters().getJSONObject("sizes").optJSONObject("large") !=
            null)) {
            d.getJSONObject("attributes").put("width", ma.getParameters().
                    getJSONObject("sizes").getJSONObject("large").optDouble("w",
                0));
            d.getJSONObject("attributes").put("height", ma.getParameters().
                    getJSONObject("sizes").getJSONObject("large").optDouble("h",
                0));
        }
        ...
        String url = ma.getParameters().optString("media_url", "__fail__").
                toLowerCase();
